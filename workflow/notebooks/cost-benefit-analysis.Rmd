...
med_auc <- perf_dat %>% filter(dataset == 'full', outcome == 'allcause') %>% 
  summarize(med_auc = median(AUC)) %>% pull(med_auc)
med_seed <- perf_dat %>% filter(dataset == 'full', outcome == 'allcause') %>% 
  mutate(rel_diff_auc = abs(AUC - med_auc)) %>% 
  slice_min(rel_diff_auc) %>% 
  pull(seed) %>% 
  .[1]
sensspec_dat <- read_csv(here('results','sensspec_results_aggregated.csv'))
dat_med <- sensspec_dat %>% filter(dataset == 'full', outcome == 'allcause', seed == med_seed) 
sensspec_dat %>% 
  filter(dataset == 'full', outcome == 'pragmatic', precision < 0.04)
```

```{r sensspec}
model <- read_rds(here('results/predict_allcause/taxlevel_OTU/metric_AUC/dataset_full/trainfrac_0.8/runs/rf_142_model.Rds'))
test_dat <- read_csv(here('results/predict_allcause/taxlevel_OTU/metric_AUC/dataset_full/trainfrac_0.8/runs/rf_142_test-data.csv'))
preds <- stats::predict(model, test_dat, type = 'prob') %>% 
  mutate(pred_pos = yes > 0.5)
```

```{r decision-thresholds}
thresh_dat <- read_csv(here('results','thresholds_results_aggregated.csv'))
thresh_dat %>% 
  filter(dataset == 'full', outcome == 'attrib', seed == 100) %>% 
