...
shared <- read_tsv(snakemake@input[["shared"]])
full_metadata <- read_csv(snakemake@input[["full_metadata"]]) %>% inner_join(shared)
int_metadata <- read_csv(snakemake@input[["int_metadata"]]) %>% inner_join(shared)
