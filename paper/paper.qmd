...
variables, achieving precision values of 0.417 (NNS = `r ehr_nns`) for the EHR
model and 0.167 (NNS = `r curated_nns`) for the curated model at the 95th
percentile of risk [@li_using_2019;@rao_clostridium_2015].
...
Thus, in a hypothetical scenario where these assumptions about fidaxomicin were
correct, between `r min_fdx_nnb` and `r max_fdx_nnb` patients would need to be
...
screened by an OTU-based prediction model in order for one patient to benefit by
treatment guided by the model.
For comparison, the NNB for pairing the prior EHR-based model with fidaxomicin
would be `r ehr_fdx_nnb` with `r ehr_screen` total CDI patients screened for
