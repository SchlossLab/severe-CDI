...
have also been shown to increase CDI susceptibilty and inhibit clearance
...
Patient risk factors for CDI-related morbidity and mortality include age > 65,
history of recurrent CDI, and co-morbid chronic illnesses [@ressler_defining_2021].
...
of features to  classify samples and predict outcomes, rather than relying on
limited sets of human-curated variables.
...
[@rao_clostridium_2015;@li_using_2019;@dieterle_systemic_2020].
...
Mouse studies have found that the initial state of the gut microbiome predicts
differences in clearance, moribundity, and cecal tissue damage in mice infected
with CDI [@tomkovich_initial_2020;@lesniak_gut_2022].
...
We set out to investigate whether ML models trained on features of the gut
microbiota can predict CDI severity in a human cohort,
...
We then trained ML models to predict each of four severity definitions from
...
[@ressler_defining_2021].
We explored four different ways to define CDI cases as severe or not (@fig-flowchart).
The "IDSA" definition of severe CDI is based on laboratory values collected on
...
However, the IDSA score is known to be a poor predictor of adverse outcomes
[@stevens_validation_2020], although it is straightforward to collect.
...
all cases, so we defined "all-cause" severity where a case is severe if an
...
definition when chart review has not been completed, allowing us to use as many
samples as we have available while taking physicians' expert opinions into
account where possible (@fig-flowchart B).
We trained ML models to classify (in the case of the IDSA definition) or predict
(in the case of the three other definitions) severity for each definition and
determined how well OTU-based models perform for each of them.
...
attributable severity, yielding an AUROC of `r ehr_auroc` [@li_using_2019].
...
The purpose of using the full datasets is to train the best models possible
given the constraints, while using the intersection dataset allows for comparing
severity definitions.
We found that models predicting pragmatic severity with as much data as
available performed best, and that performance dropped substantially when 
reducing to the intersection datasets for all definitions likely due to the
particularly imbalanced nature of the all-cause and attributable definitions.
...
severity has been well-documented in prior studies, thus its importance and
...
Overall the abundance data are sparse, as these patients were likely on
antibiotics prior to CDI onset and may have begun antibiotic treatment prior to
the initial stool sample collection.
...
in a clinical setting; benefit over current practices must be shown.
We calculated the number needed to screen for our OTU-based models and compared
these to EHR-based and curated models trained two days after the diagnosis, and
found the OTU-based pragmatic severity model outperformed the curated models but
not the EHR models.
...
Although no known treatment options are proven to reduce the risk of severe CDI
outcomes, fidaxomicin is promising due to its improved time to resolution and
reduced recurrence.
Fidaxomicin is also attractive as a preferential antibiotic option as
vancomycin-resistant enterococcus is on the rise and enterococci are known to
worsen CDI [@poduval_clostridium_2000;@smith_enterococci_2022].
...
treatment options versus the savings of averting severe outcomes, as economic
disparities are a major barrier to treatment in the US.
Cost-benefit analyses have found that fidaxomicin is more cost-effective than
vancomycin largely due to the reduced risk of recurrence [@johnson_clinical_2021;@jiang_budget_2022].

As patient age is already the most significant risk factor, the gut microbiome
may only a minor part in influencing CDI severity.
However, deploying models incorporating both microbial factors and patient
factors from the EHR could be worth the time and costs if
performance improvements were shown translate to clinically significant improvements in
patient outcomes.
in practice, EHR-based models are easier and less costly to deploy than an OTU-based model would be -- can be embedded for automated scores.
...
Future work is needed to determine whether incorporating both microbial features
and EHRs would result in a clinically meaningful improvement in performance over
deploying EHR models alone.
...
format_table <- function(tab) {
  tab %>%
    kable(col.names = c(' ', 'IDSA', 'All-cause', 'Attributable', "Pragmatic"),
