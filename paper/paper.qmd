...
must be treated by an intervention in order for one patient to benefit from the
treatment. 
...
The NNB pairs model performance with treatment effectiveness to estimate the
benefit of using predictive models in clinical practice.

We determined the decision threshold at the 95th percentile of risk for each
prediction model and computed the confusion matrix and several performance
metrics including the NNS (@tbl-risk).
...
Electronic Health Record data and from a smaller set of clinician-curated
...
relevant as physician chart review increases confidence that positively-labelled
...
for reducing severe CDI outcomes, the reduced risk of recurrence 
