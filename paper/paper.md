...
abstract: |
  ...
  We collected 1,277 stool samples from CDI patients,
  performed 16S rRNA gene amplicon sequencing, then processed and clustered the
  sequences into _de novo_ Operational Taxonomic Units.
  We trained ML models to predict CDI severity according to four different
  severity definitions: 
  the IDSA severity score on the day of diagnosis,
  all-cause adverse outcomes within 30 days,
  disease-related complications confirmed attributable to CDI via chart review, 
  ...
  The models predicting the pragmatic severity definition performed best, 
  ...
  model performance across all severity definitions. 
  ...
  Health Record data.
  Our results show that OTU-based models can modestly identify patients at risk
  for severe CDI, which could be used by healthcare providers to identify
  patients at risk and ultimately improve CDI outcomes.
#importance: TODO
...
::: {.cell hash='paper_cache/pdf/setup_9f8fc407be36817e2e940efb7905731b'}

:::

::: {.cell hash='paper_cache/pdf/load_data_fb0bfeebe26fbe4bf2f2bf79235f70aa'}
...
::: {.cell hash='paper_cache/pdf/abstract_8ecd26b6aee9c69db9e30fb6d3baf74a'}
# Abstract
...
antibiotics typical risk factor for cdi, but nonantibiotic medications can 
increase susceptibilty too [@tomkovich_osmotic_2021]
and non-nosocomial CDI on the rise
...
prediction models based on EHR for whether infection occurs in the first place already in use [@otles_clostridioides_2023].
so how about predicting severity of infections to guide treatment.
also models on EHR to predict adverse outcomes (Li, Rao).
serum-based biomarker model (Dieterle mbio 2020).
...
inform how the gut microbiome may modulate severity (ML-based science: good
performance implies something about underlying biology), and determine whether
there is potential clinical value in OTU-based models.
...
::: {.cell hash='paper_cache/pdf/model_comps_full_3abea1c72bb8be8e739bc0e2fe19ebf3'}
...
severity performed best with a median AUROC of 0.6864548,
...
(@fig-performance B).
For comparison, a prior study trained a logistic regression model on whole
Electronic Health Record data extracted on the day of CDI diagnosis to predict
attributable severity, yielding an AUROC of 0.69 [@li_using_2019].
...
0.595164 for IDSA severity,
0.6687631 for all-cause severity,
0.6584436 for attributable severity, and
0.7466801 for pragmatic severity.
...
::: {.cell hash='paper_cache/pdf/model_comps_int_d53dd1a1821c678dd89d9abe4994bf69'}
...
0.6026316 for IDSA severity,
0.5489418 for all-cause severity,
0.5865285 and for attributable severity.
...
0.593301 for IDSA severity,
0.5523027 for all-cause severity,
0.5820498 and for attributable severity.
...
TODO summarize which OTUs enriched in severe vs not severe.
...
datasets performed best with an NNS of 4 for the all-cause
definition, 6 for the attributable definition, and 
3 for the pragmatic definition.
Multiplying the NNS of the OTU-based models by the estimated NNT of 
10 for fidaxomicin yields NNB values of 40 
for all-cause severity, 60 for attributable severity,
and 30 for pragmatic severity.
Thus, in a hypothetical scenario where these assumptions about fidaxomicin hold
true, between 30 and 60 patients would need to be
...
that 600 to 1200 total CDI patients would need to be
...
(NNS = 2.3980815) for the EHR model and 0.167 (NNS = 5.988024) for the
curated model at the 95th percentile of risk
[@li_using_2019;@rao_clostridium_2015]. <!-- TODO possible to find NNS on day of diagnosis for EHR model-->
Pairing the prior EHR-based model with fidaxomicin would yield
an NNB of 24 with 480 total CDI patients screened for
one patient to benefit, although the EHR was extracted two days after diagnosis
while OTUs in this study are from stool samples collected on the day of diagnosis.
...
For many of the top OTUs, there is wide variance in importance, perhaps due to
the imbalanced nature of the severity outcomes.
Enrichment of _Enterococcus_ and _Lactobacillus_ in _C. difficile_ infection and severity has been well-documented
in prior studies, thus its importance and increase in abundance for severe cases
is not surprising 
...
vanc-resistant enterococcus new nosocomial alliance [@poduval_clostridium_2000].

Compare to EHR-based models.
in practice, EHR-based models are easier and less costly to deploy than an OTU-based model would be.
...
justified.
resistance to vancomycin is increasing in staph, cdi, and enterococci -- even
more important to find alternate treatments.
...
mothur [@westcott_opticlust_2017], resulting in 9939 OTUs.
...
During pre-processing, 9757 to 9760
...
we set out to estimate the potential benefit of deploying models in clincal
settings.
...
::: {#tbl-counts .cell layout-ncol="2" tbl-pos='h' tbl-cap='**Sample counts and proportion of severe cases.** Each severity definition has a different number of patient samples available, as well as a different proportion of cases labelled as severe. ' tbl-subcap='["Full datasets","Intersection of samples with all labels available"]' hash='paper_cache/pdf/tbl-counts_da9d0fedf368338e48ead61617227224'}

:::

::: {#tbl-risk .cell tbl-pos='h' tbl-cap='**Predictive model performance at 95th percentile of risk.** The confusion matrix was computed for the decision threshold at the 95th percentile of risk for each severity prediction model, which corresponds to 5% of cases predicted to have a severe outcome. The number needed to screen (NNS) to identify one true positive is the reciprocal of precision. ' tbl-subcap='["Full datasets","Intersection of samples with all labels available"]' hash='paper_cache/pdf/tbl-risk_9b79818ba0c79a1c752b4e2bff679223'}
...
with ribbons as the 95% CI.
Original unbalanced precision-recall curves are shown in Figure S1.
"}
